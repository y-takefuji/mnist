...
X_scaled_pca = scaler.fit_transform(X_subsample)
...
# We sum the absolute loadings across all components to get feature importance
...
# Display top 5 features from PCA (or fewer if num_features < 5)
print(f"\nTop {min(5, num_features)} PCA most influential original features:")
for i in range(min(5, num_features)):
...
# Random Forest for MI, TE, and PCA features
...
# Evaluate PCA selected features with Random Forest
...
print(f"PCA top {num_features} features (with RF) - CV Accuracy: {scores_pca.mean():.4f} ± {scores_pca.std():.4f}")
...
results = [
    ...
    ("PCA (with RF)", scores_pca.mean(), scores_pca.std())
]
